...
async function scrapeColleges(page) {
  try {
    ...
    await page.waitForSelector('div.cs-search-results-list-display', { timeout: 60000 });
    ...
    const colleges = await page.evaluate(() => {
      ...
      const collegeElements = document.querySelectorAll('div.cs-college-card-outer-container');

      // Extract relevant information from each card
      return Array.from(collegeElements).map(college => {
        const name = college.querySelector('h3.cs-college-card-college-name')?.innerText.trim() || 'N/A';  // College name
        const locationText = college.querySelector('div.cs-college-card-college-address')?.innerText.trim() || '';  // Location
        const [city, state] = locationText.split(', ').map(str => str.trim());  // Split into city and state
        const detailPageUrl = college.querySelector('a.cs-college-card-college-name-link')?.href || null;  // URL to detail page
