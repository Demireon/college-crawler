...
async function scrapeCollegeDetails(page, college) {
  try {
    ...
    await page.goto(college.detailPageUrl, {
    ...
    logMessage(
      "info",
      `Scraped College Board Code for ${college.name}: ${
        collegeBoardCode || "Not Found"
      }`
    );
    ...
    logMessage(
      "error",
      `Error scraping details for ${college.name}: ${error.message}`
    );
