  ...
  try {
    ...
    while (true) {
      ...
      for (const college of colleges) {
        ...
        logMessage(
          "info",
          `Waiting for ${delay} milliseconds before processing next college...`
        );
        ...
        try {
          ...
          if (!existingCollege) {
            ...
            logMessage(
              "info",
              `College ${fullCollegeData.name} already exists in the database`
            );
          }
        } catch (dbError) {
          logMessage(
            "error",
            `Error saving data for ${fullCollegeData.name}: ${dbError.message}`
          );
        }
      ...
      const showMoreButton = await page.$(
        'button[data-testid="cs-show-more-results"]'
      );
      if (showMoreButton) {
        ...
        logMessage(
          "info",
          `Waiting for ${delay} milliseconds before clicking "Show More Colleges" button.`
        );
        await new Promise((resolve) => setTimeout(resolve, delay));

        // Get the current number of college cards
        const previousCollegeCount = await page.evaluate(
          () =>
            document.querySelectorAll("div.cs-college-card-outer-container")
              .length
        );
        ...
        logMessage(
          "info",
          'Clicked "Show More Colleges" button to load more colleges...'
        );
      } else {
        logMessage(
          "info",
          "Show More Colleges button not found. Exiting loop."
        );
